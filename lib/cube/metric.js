...
exports.getter = function(db) {
  ...
  function getter(request, callback) {
    ...
    function query(callback) {
      ...
      measure(expression, filter.start, filter.stop, tier, "id" in request
    ...
    if (stream) {
      ...
      if (streams) {
        filter.stop = streams.time;
        streams.waiting.push({
          id: id,
          callback: callback
        });
        query(function(metric) { if (metric.value) callback(metric); });
      }
        ...
        (function poll() {
          query(function (metric) {
            if (metric.value !== undefined) {
              ...
              filter.start = streams.time;
              filter.stop = streams.time = tier.ceil(+streams.time + tier.key);
