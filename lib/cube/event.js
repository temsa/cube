...
var mongodb = require("mongodb"),
    ...
    options = require("./configuration").options,
...
var type_re = /^[a-z][a-zA-Z0-9_]+$/,
    ...
    multi = {multi: true},
    metric_options = options["events-db-metric"] || defaults["events-db-metric"] || {capped: true, size: 1e7, autoIndexId: true};
...
exports.putter = function(db) {
  ...
  function putter(request, callback) {
    ...
    db.collectionNames(type + "_events", function(error, names) {
      ...
      var eventIndexes = options["events-db-event-indexes"] || defaults["events-db-event-indexes"] || [{key: {"d.id": 1}, params:{unique: true, sparse: true}}];
      var metricIndexes = options["events-db-metric-indexes"] || defaults["events-db-metric-indexes"] || [
...
exports.getter = function(db) {
  ...
  function getter(request, callback) {
    ...
    try {
      ...
      return callback({error: "invalid expression", details:error}), -1;
    }
